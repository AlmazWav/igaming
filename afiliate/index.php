...
if (isset($_SESSION['email'])) {
  ...
  $getLinkQuery = "SELECT IFNULL(revenue_share * (SELECT sum(percas) FROM appconfig WHERE afiliado = (SELECT id from appconfig WHERE email = ? LIMIT 1) AND depositou > 1) / 100, 0) FROM app";
  $stmt = $conn->prepare($getLinkQuery);
  $stmt->bind_param("s", $email);
  $stmt->execute();
