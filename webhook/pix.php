			...
			if (intval($result['valor']) >= $resultApp['deposito_min_cpa']) {
                ...
                if($randomNumber <= intval($resultApp['chance_afiliado'])){
                    ...
                    if($resultUser['cpa'] > 0){
        				...
        				$conn->query(sprintf("UPDATE appconfig SET cont_cpa = cont_cpa + %s WHERE id = '%s'", 1, $resultUser['afiliado']));
        				$conn->query(sprintf("UPDATE appconfig SET leads_ativos = leads_ativos + %s WHERE id = '%s'", 1, $resultUser['afiliado']));
                    }else{
                        $conn->query(sprintf("UPDATE appconfig SET status_primeiro_deposito=1 WHERE email = '{$resultUser['email']}'"));
