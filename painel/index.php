...
$getApostaMin = "SELECT aposta_min FROM app";
  $stmt = $conn->prepare($getApostaMin);
  $stmt->execute();
  $stmt->bind_result($apostaMin);
  $stmt->fetch();
  $stmt->close();
...
<form id="formSubtrair1" method="post" aria-label="Form" action="/jogar/aguarde.php">
<h4 class="rarity-heading">Valor da Aposta:</h4>    
<div class="rarity-row roboto-type2">
<input type="number" class="large-input-field w-node-_050dfc36-93a8-d840-d215-4fca9adfe60d-9adfe605 w-input" maxlength="3" min="<?php echo $apostaMin?>" max="100" name="valoraposta" placeholder="Insira um valor a ser apostado" id="valoraposta" required="">
<input type="submit" value="APOSTAR" class="primary-button w-button">
</div>
</form>
